...
model=load_model('SaveModelData\\geo_engDigit.h5')
model.summary()
y_pred='noname'
mask=0
...
ws = websocket.WebSocketApp("ws://192.168.43.93:8000",  on_message = on_message,  on_error = on_error, on_close = on_close)
ws.on_open = on_open
...
def interact():
    ...
    if v==con1:
        ...
        time.sleep(1)
        data={'type':'tts', 'data': u' লাইন বর্গ ত্রিভুজ বৃত্ত ও ইংরেজী সংখ্যা লিখতে পারি '}
        data = json.dumps(data)
        ws.send(data)
...
if __name__ == '__main__':
    root=Tk()
    button2=Button(root,text='Capture',command=predict).place(x=15,y=50)
    button3=Button(root,text='Interaction',command=interact).place(x=100,y=50)
